  ...
  <div style="max-width:640px;margin:0 auto;padding:24px;">
    <div style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 8px 24px rgba(0,0,0,.08);">
      ...
      <div style="padding:12px 20px;font-family:Arial,Helvetica,sans-serif;color:#6b7280;font-size:12px;border-top:1px solid #e5e7eb;">
        Ova poruka je poslata sa bowencentar.rs
      </div>
    </div>
  </div>
</body>
...
if (isset($_GET['confirm'], $_GET['type'], $_GET['token'])) {
    ...
    if (!$db) {
        ...
        header('Location: kontakt.php#' . (($_SESSION['msg_target'] === 'unsubscribe') ? 'unsubscribe-form' : (($_SESSION['msg_target'] === 'delete') ? 'delete-form' : 'newsletter-form')));
        exit;
    }
    $db_host = $db['host'];
    $db_name = $db['name'];
    ...
    if (!$stmt) {
        ...
        $_SESSION['error_msg'] = 'Greška pri pripremi upita.';
        header('Location: kontakt.php#newsletter-form');
        exit;
    }
    ...
    if (!$row) {
        $_SESSION['error_msg'] = 'Nevažeći ili istekao link.';
        header('Location: kontakt.php#newsletter-form');
        exit;
    }
    if (!empty($row['used_at'])) {
        $_SESSION['error_msg'] = 'Ovaj link je već iskorišćen.';
        header('Location: kontakt.php#newsletter-form');
        exit;
    }
    $expires = strtotime($row['expires_at']);
    if ($expires !== false && $expires < time()) {
        $_SESSION['error_msg'] = 'Link je istekao. Pošaljite novi zahtev.';
        header('Location: kontakt.php#newsletter-form');
        exit;
    }
    if ($row['action'] !== $type) {
        $_SESSION['error_msg'] = 'Neispravan tip akcije.';
        header('Location: kontakt.php#newsletter-form');
        exit;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $db_pass = $db['pass'];
        ...
        if ($stmt->execute()) {
            ...
            $_SESSION['msg_target'] = 'subscribe';
                ...
                $_SESSION['error_msg'] = 'Došlo je do greške pri upisu.';
                $_SESSION['form'] = ['ime'=>$ime,'prezime'=>$prezime,'email'=>$email,'datum_rodjenja'=>$datum,'newsletter'=>$newsletter];
            }
        }
    ...
    if ($action === 'unsubscribe_request') {
        ...
        if ($u_email === '' || !filter_var($u_email, FILTER_VALIDATE_EMAIL)) {
            ...
            header('Location: kontakt.php#unsubscribe-form');
            exit;
        }
    ...
    $db_pass = $db['pass'];
        ...
        if (!send_mail_html($u_email, "Potvrda odjave sa newsletter-a", $body)) {
            $_SESSION['error_msg'] = 'Nije moguće poslati email. Proverite inbox/spam ili mail() podešavanja na hostingu.';
            $_SESSION['unsub_form'] = ['email'=>$u_email];
            header('Location: kontakt.php#unsubscribe-form');
            exit;
        }
    ...
    if ($action === 'delete_request') {
        ...
        if ($d_email === '' || !filter_var($d_email, FILTER_VALIDATE_EMAIL)) {
            ...
            header('Location: kontakt.php#delete-form');
            exit;
        }
    ...
    <div class="page_wrap">
        ...
        <section class="top_panel_image">
            ...
            <div class="top_panel_image_header">
                ...
                <h1 class="top_panel_image_title entry-title">Bowen centar - Zakažite terapiju</h1>
                                        ...
                                        <div class="wrapper">
                                            <div class="sc_section sc_section_block cu_mrg_5">
                                                <div class="sc_section_inner">
                                                    ...
                                                    <h5 class="sc_title sc_title_regular sc_align_center">Bowen centar se nalazi u Beogradu, u blizini Autokomande.  </h5>
                                                    <h5 class="sc_title sc_title_regular sc_align_center">Veoma pristupačna lokacija iz svih delova grada, bilo da dolazite gradskim prevozom ili kolima. Parking zone nema. </h5>
                                                    <div class="sc_section_content_wrap">
                                                        <div class="columns_wrap sc_columns columns_nofluid">
                                                            <div class="column-1_3 sc_column_item">
                                                                <h5 class="sc_title sc_title_regular sc_align_center">O Bowen centru</h5>
                                                                <div class="text_column content_element ">
                                                                    <div class="wrapper">
                                                                        <p>Vaše telo zna kako da se regeneriše, Vi mu samo dajte priliku. Oslobodite se napotosti, umirite um i zakažite svoj Bowen tretman već danas!</p>
                                                                    </div>
                                                                </div>
                        ...
                        <div class="sc_section_content_wrap">
                            ...
                            <form method="post" action="kontakt.php" class="sc_form sc_form_style_form_3">
                                ...
                                <div class="sc_form_info">
                                    ...
                                    <div class="sc_form_item sc_form_field label_over">
                                        <input type="text" name="ime" placeholder="Ime" value="<?php echo htmlspecialchars($ime, ENT_QUOTES, 'UTF-8'); ?>" required>
                                    </div>

                                    <div class="sc_form_item sc_form_field label_over">
                                        <input type="text" name="prezime" placeholder="Prezime" value="<?php echo htmlspecialchars($prezime, ENT_QUOTES, 'UTF-8'); ?>" required>
                                    </div>
...
<div class="newsletter-subtitle">Datum rođenja nije obavezan. Ako ga unosite, izaberite dan/mesec/godinu.</div>
                                ...
                                <div class="sc_form_item sc_form_button" style="text-align:center;">
                                    <button class="sc_button sc_button_style_filled sc_button_size_medium">
                                        <span class="overlay"><span class="first">Pošalji podatke i prijavi se za newsletter</span><span class="second">Pošalji podatke i prijavi se za newsletter</span></span></button>
                                </div>
...
<h3 id="unsubscribe-form" class="sc_title sc_title_regular cu_mrg_5" style="font-size:24px;">Odjava sa newsletter-a</h3>
    ...
    <div class="sc_section_content_wrap">
      ...
      <form method="post" action="kontakt.php" class="sc_form sc_form_style_form_3" style="margin-top:10px;">
        ...
        <div class="sc_form_info">
          <div class="sc_form_item sc_form_field label_over">
            <input type="email" name="unsub_email" placeholder="Unesite email adresu" value="<?php echo htmlspecialchars($unsub_email, ENT_QUOTES, 'UTF-8'); ?>" required>
          </div>
        </div>
        ...
        <div class="sc_form_item sc_form_button" style="display:flex; justify-content:center;">
          <button class="sc_button sc_button_style_filled sc_button_size_medium" type="submit">
            <span class="overlay"><span class="first">Pošalji link za odjavu</span><span class="second">Pošalji link za odjavu</span></span></button>
        </div>

        <div style="font-size:14px; opacity:.9; text-align:center; margin-top:8px;">
          Na email će vam stići link za potvrdu odjave (važi 24h).
        </div>
      </form>
...
<h3 id="delete-form" class="sc_title sc_title_regular cu_mrg_5" style="font-size:24px;">Izbriši moje podatke</h3>
    ...
    <div class="sc_section_content_wrap">
      ...
      <form method="post" action="kontakt.php" class="sc_form sc_form_style_form_3" style="margin-top:10px;">
        ...
        <div class="sc_form_info">
          <div class="sc_form_item sc_form_field label_over">
            <input type="email" name="del_email" placeholder="Unesite email adresu" value="<?php echo htmlspecialchars($del_email, ENT_QUOTES, 'UTF-8'); ?>" required>
          </div>
        </div>

        <div class="sc_form_item sc_form_button" style="display:flex; justify-content:center;">
          <button class="sc_button sc_button_style_filled sc_button_size_medium" type="submit">
            <span class="overlay"><span class="first">Pošalji link za brisanje</span><span class="second">Pošalji link za brisanje</span></span></button>
        </div>

        <div style="font-size:14px; opacity:.9; text-align:center; margin-top:8px;">
          Na email će vam stići link za potvrdu brisanja (važi 24h). Ova akcija je nepovratna.
        </div>
      </form>
...
(function(){
  ...
  var subscribeForm = document.querySelector('form[action="kontakt.php"][method="post"] input[name="action"][value="subscribe"]');
  if(subscribeForm){
